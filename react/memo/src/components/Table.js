...
class Table extends Component {
  ...
  getToken = async () => {
    try {
      // Récupération du token qui sera stocké dans l'instance coop de Coopernet
      await this.coop.getToken();
    }
    catch (error) {
      console.log('Erreur de token dans getToken de Table : ', error);
      error.message = "Impossible de récupérer le token via la fonction fetch";
  ...
  getUserTerms = async (login, pwd) => {
    try {
      ...
      this.user = await this.coop.getUser(login, pwd);
      if (!this.user) throw new Error("Erreur de login ");
      console.log('user dans  getUserTerms de table : ', this.user);

      // Récupération des rubriques (terms ex : HTML) 
      const terms = await this.coop.getTerms();
      ...
      state_copy.user_connected = true;
  ...
  addCard = async (new_card, col_index, card_index) => {
    ...
    const card_database_id = await this.coop.addCard(
      new_card, 
  ...
  handleClickTerm = async term => {
    ...
    const columns = await this.coop.getCards(term.id);
